...
class TabbedWeatherDashboard:
    ...
    def restyle_all_components(self):
        ...
        if hasattr(self, 'about_component') and hasattr(self.about_component, "refresh"):
            try:
                self.about_component.refresh()
            except Exception as e:
                self.logger.error(f"Error refreshing about component: {e}")
