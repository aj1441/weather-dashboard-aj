...
from gui.shared.icon_manager import get_weather_icon
from utils.conversion.weather_data_parser import parse_weather_data
...
class WeatherDisplayComponent:
    ...
    def setup_component(self) -> tb.Frame:
        ...
        from utils.data.save_city_utils import create_save_city_button
    ...
    def update_display(self, weather_data: Dict[str, Any], used_fallback: bool = False) -> None:
        ...
        try:
            ...
            # Format temperature and unit label
            temp = weather_data.get('temperature')
            # Accept both "unit" (e.g., "°F"/"°C") and fallback to correct label if missing
            unit = weather_data.get('unit')
            if not unit:
                # Try to infer from possible 'units' field or fallback to °F
                units_mode = weather_data.get('units', 'imperial')
                unit = '°C' if units_mode == 'metric' else '°F'
            if temp is not None:
